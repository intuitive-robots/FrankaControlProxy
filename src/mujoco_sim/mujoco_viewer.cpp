...
MujocoViewer::MujocoViewer(MujocoPandaEnv* env) : env_(env) {}
...
void MujocoViewer::start(int width, int height, const char* title)
{
    ...
    render_thread_ = std::thread(&MujocoViewer::renderLoop, this, width, height, title);
}
...
void MujocoViewer::renderLoop(int width, int height, const char* title)
{
    ...
    while (running_)
    {
        ...
        glfwPollEvents();
        next += std::chrono::microseconds(static_cast<int>(render_dt_ * 1'000'000));
...
void MujocoViewer::handleCursorPos(double xpos, double ypos)
{
    ...
    {
        mjv_moveCamera(model_, mjMOUSE_ROTATE_H, scale * dx, scale * dy, &scn_, &cam_);
    }
    else if (mouse_state_.right_down)
    {
        mjv_moveCamera(model_, mjMOUSE_MOVE_H, scale * dx, scale * dy, &scn_, &cam_);
    }
}
