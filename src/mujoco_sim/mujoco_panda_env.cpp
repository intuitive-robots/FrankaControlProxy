...
MujocoPandaEnv::MujocoPandaEnv(const std::string& model_path)
...
void MujocoPandaEnv::nextStep(const franka::Torques& torques, franka::RobotState& robot_state) {
  ...
  updateRobotState(robot_state);
}


void MujocoPandaEnv::updateRobotState(franka::RobotState& robot_state) {
  std::lock_guard<std::mutex> lock(state_mutex_);
