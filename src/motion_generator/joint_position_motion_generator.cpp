...
JointPositionMotionGenerator::JointPositionMotionGenerator(
    ...
    AtomicDoubleBuffer<franka::RobotState>& state_buffer, double tolerance)
    ...
    const franka::RobotState& robot_state, franka::Duration period)
{
    ...
    JointPositionMotionGenerator::Vector7d delta_q_d;
...
bool JointPositionMotionGenerator::calculateDesiredValues(
    double time, JointPositionMotionGenerator::Vector7d* delta_q_d) const
{
    JointPositionMotionGenerator::Vector7i sign_delta_q;
    sign_delta_q << delta_q_.cwiseSign().cast<int>();
    JointPositionMotionGenerator::Vector7d t_d = t_2_sync_ - t_1_sync_;
    JointPositionMotionGenerator::Vector7d delta_t_2_sync = t_f_sync_ - t_2_sync_;
        ...
        else
        {
            if (time < t_1_sync_[i])
            {
                (*delta_q_d)[i] = -1.0 / std::pow(t_1_sync_[i], 3.0) * dq_max_sync_[i] *
                                  sign_delta_q[i] * (0.5 * time - t_1_sync_[i]) *
                                  std::pow(time, 3.0);
            }
            else if (time >= t_1_sync_[i] && time < t_2_sync_[i])
            {
                (*delta_q_d)[i] =
                    q_1_[i] + (time - t_1_sync_[i]) * dq_max_sync_[i] * sign_delta_q[i];
            }
...
void JointPositionMotionGenerator::calculateSynchronizedValues()
{
    JointPositionMotionGenerator::Vector7d dq_max_reach(dq_max_);
    JointPositionMotionGenerator::Vector7d t_f = JointPositionMotionGenerator::Vector7d::Zero();
    JointPositionMotionGenerator::Vector7d delta_t_2 =
        JointPositionMotionGenerator::Vector7d::Zero();
    JointPositionMotionGenerator::Vector7d t_1 = JointPositionMotionGenerator::Vector7d::Zero();
    JointPositionMotionGenerator::Vector7d delta_t_2_sync =
        JointPositionMotionGenerator::Vector7d::Zero();
    JointPositionMotionGenerator::Vector7i sign_delta_q;
    sign_delta_q << delta_q_.cwiseSign().cast<int>();
