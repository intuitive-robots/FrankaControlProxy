...
CartesianPoseMotionGenerator::CartesianPoseMotionGenerator(
    double speed_factor, const Eigen::Vector3d& goal_position,
    const Eigen::Quaterniond& goal_orientation,
    AtomicDoubleBuffer<franka::RobotState>& state_buffer, double tolerance, double dx_max,
    double ddx_max_start, double ddx_max_goal, double omega_max, double domega_max_start,
    ...
    const franka::RobotState& robot_state, franka::Duration period)
{
    ...
    if (time_ == 0.0)
    {
        ...
        pos_start_ =
            Vector3d(robot_state.O_T_EE[12], robot_state.O_T_EE[13], robot_state.O_T_EE[14]);
...
bool CartesianPoseMotionGenerator::calculateDesiredValues(double time, Vector3d* delta_pos_d,
                                                          double* alpha) const
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        else
        {
            ...
            {
                (*delta_pos_d)[i] = -1.0 / std::pow(t_1_sync_pos_[i], 3.0) * dx_max_sync_[i] *
                                    sign_delta_pos[i] * (0.5 * time - t_1_sync_pos_[i]) *
                                    std::pow(time, 3.0);
            }
            else if (time >= t_1_sync_pos_[i] && time < t_2_sync_pos_[i])
            {
                (*delta_pos_d)[i] =
                    pos_1_[i] + (time - t_1_sync_pos_[i]) * dx_max_sync_[i] * sign_delta_pos[i];
            }
                ...
                (*delta_pos_d)[i] =
                    ...
                    0.5 *
                        (1.0 / std::pow(delta_t_2_sync_pos[i], 3.0) *
                         ...
                         (2.0 * time - 2.0 * t_1_sync_pos_[i] - delta_t_2_sync_pos[i] -
                          2.0 * t_d_pos[i])) *
...
void CartesianPoseMotionGenerator::calculateSynchronizedValues()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        if (std::abs(delta_pos_[i]) > kDeltaPosMotionFinished)
        {
            if (std::abs(delta_pos_[i]) <
                (3.0 / 4.0 * (std::pow(dx_max_[i], 2.0) / ddx_max_start_[i]) +
            ...
            t_f_pos[i] = t_1_pos[i] / 2.0 + delta_t_2_pos[i] / 2.0 +
                         std::abs(delta_pos_[i]) / dx_max_reach[i];
        }
    }
    ...
    if (delta_rot_ > kDeltaRotMotionFinished)
    {
        ...
        {
            omega_max_reach =
                std::sqrt(4.0 / 3.0 * delta_rot_ * (domega_max_start_ * domega_max_goal_) /
    ...
    if (delta_rot_ > kDeltaRotMotionFinished)
    {
        ...
        t_f_sync_rot_ =
            t_1_sync_rot_ / 2.0 + delta_t_2_sync_rot / 2.0 + delta_rot_ / omega_max_sync_;
...
std::array<double, 16> CartesianPoseMotionGenerator::poseToMatrix(
    const Vector3d& position, const Eigen::Quaterniond& orientation) const
{
