...
namespace transform
{
    ...
    Pose::Pose(const std::array<double, 16>& pose) : Pose(array2eigen<4, 4>(pose)) {}
    ...
    Pose::Pose(const Eigen::Vector4d& quaternion, const Eigen::Vector3d& translation)
    {
        this->m_translation = translation;
    ...
    Pose::Pose(const Eigen::Quaterniond& rotation, const Eigen::Vector3d& translation)
    {
        this->m_translation = translation;
    ...
    Pose::Pose(const Eigen::Vector3d& rotation, const Eigen::Vector3d& translation)
    {
        this->m_translation = translation;
    ...
    Eigen::Vector4d Pose::rotation_q() const
    {
        return this->m_rotation.coeffs();
    }
    ...
    {
        Eigen::Affine3d pose = Eigen::Translation3d(this->m_translation) * this->m_rotation;
        return pose;
    }
    ...
    {
        // ZYX, roll, pitch, yaw
        Eigen::Vector3d rpy_vec = this->m_rotation.toRotationMatrix().eulerAngles(2, 1, 0);
    ...
    Pose Pose::interpolate(const Pose& dest_pose, double progress) const
    {
        ...
        Eigen::Vector3d pos_result =
            this->translation() + (dest_pose.translation() - translation()) * progress;
    ...
    std::string Pose::str() const
    {
        ...
        ss << "roll: " << angles.roll << "\tpitch: " << angles.pitch << "\tyaw: " << angles.yaw;
        return ss.str();
    }

    Pose Pose::operator*(const Pose& pose_b) const
    {
        Eigen::Vector3d trans = this->m_rotation * pose_b.translation() + this->m_translation;
    ...
    Pose Pose::limit_rotation_angle(double max_angle) const
    {
        ...
        {
            auto new_rot = IdentityRotQuat().slerp(max_angle / curr_angle, this->m_rotation);
