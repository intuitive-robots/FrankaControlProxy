      ...
      config_(config_path)
{
    ...
    bool connected =
        gripper_->connect(config_.port, config_.baud, config_.scale_alpha, config_.scale_beta);
    if (!connected)
    {
    ...
    zlc::info("RobotiqFeedback after init: Pos {:.2f}, CmdPos {}, Curr {:.2f}", gs.position,
              gs.raw_commanded_position, gs.current);
}
...
bool RobotiqGripper::commandChanged(const RobotiqGraspCommand& a,
                                    const RobotiqGraspCommand& b) const
{
    ...
    while (is_running_)
    {
        ...
        if (gripper_)
        {
            try
            {
                ...
                {
                    zlc::info(
                        "RobotiqGripper executing new command: Pos {:.2f}, Speed {:.2f}, Force "
                        "{:.2f}, Blocking {}",
                        cmd.position, cmd.speed, cmd.force, cmd.blocking);
                    gripper_->set_gripper_position(cmd.position, cmd.speed, cmd.force,
                                                   cmd.blocking);
                    last_cmd = cmd;
                }
            }
    ...
    while (is_running_)
    {
        if (gripper_)
        {
            ...
            RobotiqGripperStateMsg gsm{
                gs.commanded_position,     last_cmd.speed, last_cmd.force, gs.position, gs.current,
                gs.raw_commanded_position, gs.raw_position};
...
void RobotiqGripper::updateCommand(const RobotiqGraspCommand& cmd)
{
    command_.write(cmd);
    zlc::info(
        "RobotiqGripper received new command: Pos {:.2f}, Speed {:.2f}, Force {:.2f}, Blocking {}",
        cmd.position, cmd.speed, cmd.force, cmd.blocking);
}
