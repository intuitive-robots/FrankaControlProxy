...
bool FrankaArmProxy::start() {
    ...
    if (type_ == "Arm") {
        if(follower_ == "true" || follower_ == "True") {
            ...
            state_sub_thread_ = std::thread(&FrankaArmProxy::stateSubscribeThread, this);
        ...
        if(follower_ == "true" || follower_ == "True") {
            gripper_sub_thread_ = std::thread(&FrankaArmProxy::gripperSubscribeThread, this);
...
bool FrankaArmProxy::startArm(){
    ...
    // }
    // control_thread_ = std::thread(&FrankaArmProxy::controlLoopThread, this);
...
void FrankaArmProxy::stop() {
    std::cout << "[INFO] Stopping FrankaArmProxy...\n";
    if (type_ == "Arm") {
        return stopArm();
...
void FrankaArmProxy::stopGripper() {
    in_control_ = false;
    // try close ZeroMQ sockets
    try {
...
franka::RobotState FrankaArmProxy::getCurrentState() {
    std::lock_guard<std::mutex> lock(control_mutex_);
    if (current_mode_) {
...
protocol::FrankaGripperState FrankaArmProxy::getCurrentGripper(){
...
void FrankaArmProxy::registerControlMode(const std::string& mode, std::unique_ptr<AbstractControlMode> control_mode) {
...
void FrankaArmProxy::statePublishThread() {
    while (in_control_) {
        ...
        //std::cout << "[FrankaArmProxy] encodeStateMessage took " << duration_us << " us" << std::endl;
        std::this_thread::sleep_for(std::chrono::milliseconds(1000 / STATE_PUB_RATE_HZ));
        //std::this_thread::sleep_for(std::chrono::milliseconds(1000));
    }
}

void FrankaArmProxy::gripperPublishThread() {
    while (in_control_) {
        ...
        auto duration_us = std::chrono::duration_cast<std::chrono::microseconds>(t_end - t_start).count();
        //check the speed of decode and pack the message
        //-std::cout << "[FrankaArmProxy] encodeGripperMessage took " << duration_us << " us" << std::endl;
...
void FrankaArmProxy::stateSubscribeThread() {
    while (in_control_) {
        try{
            zmq::message_t message;
            auto result = sub_arm_socket_.recv(message, zmq::recv_flags::none);
            if (!result) {
                std::cerr << "[FrankaArmProxy] Failed to receive state message." << std::endl;
            ...
            //std::cout << "[FrankaArmProxy] Received state update: size = " << data.size() << std::endl;
            ...
            if (protocol::decodeStateMessage(data, arm_state)) {
                ...
                //std::cout << "[FrankaArmProxy] Received state update from leader." << std::endl;
                }
            }
            else{
                std::cerr << "[FrankaArmProxy] Failed to decode state message." << std::endl;
            }
        } catch (const zmq::error_t& e) {
            std::cerr << "[FrankaArmProxy] ZMQ recv error: " << e.what() << std::endl;
            break;
        }
    }
}
...
void FrankaArmProxy::gripperSubscribeThread() {
    while (in_control_) { 
        try{
            zmq::message_t message;
            auto result = sub_gripper_socket_.recv(message, zmq::recv_flags::none);
            if(!result) std::cerr << "[FrankaArmProxy] Failed to receive gripper message." << std::endl;
            std::vector<uint8_t> data(static_cast<uint8_t*>(message.data()), static_cast<uint8_t*>(message.data()) + message.size());
            // Debug: Print the received message size
            //std::cout << "[FrankaArmProxy] Received gripper update: size = " << data.size() << std::endl;
            ...
            if (protocol::decodeGripperMessage(data, gripper_state)) {
                ...
                std::cout << "[FrankaArmProxy] Received gripper update from leader." << std::endl;
            } else {
                std::cerr << "[FrankaArmProxy] Failed to decode gripper message." << std::endl;
            }
        }catch (const zmq::error_t& e) {
            std::cerr << "[FrankaArmProxy] ZMQ recv error: " << e.what() << std::endl;
        }
...
void FrankaArmProxy::controlLoopThread() {
...
void FrankaArmProxy::handleServiceRequest(const std::vector<uint8_t>& request, std::vector<uint8_t>& response) {
    ...
    std::cout << "[FrankaArmProxy] Response prepared, size = " << resp.size() << std::endl;
