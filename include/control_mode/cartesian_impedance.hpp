...
struct CartesianImpedanceConfig : public ControllerConfig
{
    ...
    void fromFile(const std::string& controller_config_path) override
    {
        ...
        const std::array<double, 7> static_q_task_vals = reader.getArray<double, 7>("static_q_task");
        static_q_task = Eigen::Matrix<double, 7, 1>::Map(static_q_task_vals.data());
        
        const std::array<double, 7> residual_mass_vec_vals = reader.getArray<double, 7>("residual_mass_vec");
        ...
        const std::array<double, 7> avoidance_weights_vals = reader.getArray<double, 7>("avoidance_weights");
...
{
public:
    explicit CartesianImpedanceController(const SafetyLimitConfig& safety_config, const std::string& robot_name)
