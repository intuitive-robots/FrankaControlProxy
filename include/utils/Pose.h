...
namespace transform
{
    ...
    typedef Vector6d JointPosition;
 ...
 */
    ...
    std::array<double, N * M> eigen2array(Eigen::Matrix<double, N, M, Eigen::ColMajor> matrix)
    {
 ...
 */
    template <auto N, auto M>
    Eigen::Matrix<double, N, M, Eigen::ColMajor> array2eigen(std::array<double, N * M> array)
    {
    ...
    struct RPY
    {
        ...
        {
            Eigen::Vector3d rotation((Eigen::Vector3d() << roll, pitch, yaw).finished());
            return rotation;
        }
    ...
    struct RotVec
    {
        ...
        RotVec(const Eigen::Vector3d& vec) : angle_axis(vec.norm(), vec.normalized()) {}
 ...
 */
        ...
        Pose(const Eigen::Quaterniond& quaternion, const Eigen::Vector3d& translation);
 ...
 */
        bool is_close(const Pose& other, double eps_r = 1e-8, double eps_t = 1e-8) const;
    };
} // namespace transform
