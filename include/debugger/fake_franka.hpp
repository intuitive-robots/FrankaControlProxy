...
public:
    ...
    void control(ControllerCallback callback, bool limit_rate = true, double cutoff_frequency = (100.0)) {
    ...
    template<typename ControllerCallback>
    void control(ControllerCallback callback, franka::ControllerMode mode, bool limit_rate = true, double cutoff_frequency = (100.0)) {
        control(callback, limit_rate, cutoff_frequency);
    }

    template<typename MotionGeneratorCallback, typename TorqueControlCallback>
    void control(MotionGeneratorCallback motion_callback, TorqueControlCallback torque_callback, bool limit_rate = true, double cutoff_frequency = (100.0)) {
        zlc::info("FakeFrankaRobot: control loop started.");
        isOnControlLoop = true;
        while (isOnControlLoop) {
            franka::RobotState state = current_state_; 
